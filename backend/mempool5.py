...
gpData.columns = head
txDataHashP = pd.DataFrame(gpData.groupby('minGasPrice').count().reset_index())
...
for index, row in txDataHashP.iterrows():
    predictTable.loc[predictTable['gasPrice'] >= row['minGasPrice'], 'hashP'] = predictTable['hashP'].apply(lambda x: x+ row['count'])
...
'''
predictTable['intercept'] = 2.6802
predictTable['hashPowerCoef'] = -0.0235
predictTable['txAtAboveCoef']= 0.0004 

predictTable['sum'] = predictTable['intercept'] + (predictTable['hashPowerCoef'] * predictTable['hashpPct']) + (predictTable['txAtAboveCoef'] * predictTable['txAtAbove'])
...
predictTable['expectedTime'] = predictTable['expectedTime'].apply(lambda x: np.round(x, decimals=2))
...
predictTable['maxBlocks'] = predictTable['expectedWait'].apply(lambda x: x*2.5)
predictTable['maxBlocks'] = predictTable['maxBlocks'].apply(lambda x: np.round(x, decimals=2))
predictTable['maxWait'] = predictTable['maxBlocks']* predictTable['blockInterval']/60
predictTable['maxWait'] = predictTable['maxWait'].apply(lambda x: np.round(x, decimals=2))
...
def getSafeLow():
    ...
    safeLow = series.min()
    minHashList = predictTable.loc[predictTable['hashpPct']>=1.5, 'gasPrice']
...
def getAverage():
    ...
    average = series.min()
    minHashList = predictTable.loc[predictTable['hashpPct']>35, 'gasPrice']
...
def getFast():
    ...
    fastest = series.min()
    minHashList = predictTable.loc[predictTable['hashpPct']>90, 'gasPrice']
...
def getFastest():
    ...
    fastest = series.min()
    minHashList = predictTable.loc[predictTable['hashpPct']>95, 'gasPrice']
    if fastest < minHashList.min():
        fastest = minHashList.min()
    return (fastest) 
