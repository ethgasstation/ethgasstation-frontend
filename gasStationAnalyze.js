...
function writeAPIandCALC (){

       
        //Create gasAPI /Calculator
    const execFile = require('child_process').execFile;
    const child = execFile('ethgasAPI.php', (error, stdout, stderr) => {
        if (error) {
            throw error;
        }
        console.log(stdout);
...
function getETHprice (){
    ...
    callback = function(response) {
        ...
        if (response.statusCode != 200){
            return;}  //Don't post new data if prices are down
...
// Monitor blocks for periodic analysis
var startSelect, toBlock, start2;
var protect = 0;
var toBlock= 0;
...
var filter = web3.eth.filter('latest');
    ...
    var pruneBlock = currentBlock - 15000;
    
    //keep database from getting too large

    if (currentBlock % 5000 === 0){
    ...
    if ((currentBlock % 100)===0){
        toBlock = currentBlock-(currentBlock%10);
        startSelect = toBlock - 10000;
        start2 = toBlock - 100;
