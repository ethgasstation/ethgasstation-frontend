...
predictData['pctGasOffered'] = predictData['gasOffered'].apply(lambda x: x/4710000)

transactionGas = float(21000)/4710000

quantiles= predictData['pctGasOffered'].quantile([.5, .75, .9, 1])

#dep['gasCat1'] = (txData2['gasused'] == 21000).astype(int)
predictData['gasCat2'] = ((predictData['pctGasOffered']>transactionGas) & (predictData['pctGasOffered']<=quantiles[.75])).astype(int)
predictData['gasCat3'] = ((predictData['pctGasOffered']>quantiles[.75]) & (predictData['pctGasOffered']<=quantiles[.9])).astype(int)
predictData['gasCat4'] = (predictData['pctGasOffered']> quantiles[.9]).astype(int)

print('median pctGasOffered')
